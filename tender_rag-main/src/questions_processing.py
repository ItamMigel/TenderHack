...
class QuestionsProcessor:
    def __init__(
        ...
        answering_model: str = "bit14-2024-08-06",
    ...
    def _extract_references(self, pages_list: list, company_name: str) -> list:
        # По умолчанию используем имя компании как идентификатор
        company_sha1 = company_name
        
        # Пробуем найти SHA1 в subset.csv, если он есть
        if self.subset_path is not None and self.subset_path.exists():
    ...
    #         retrieval_results = retriever.retrieve_all(company_name=company_name)
    #     else:           
    ...
    #     )
    #     self.response_data = self.bit14_processor.response_data
    ...
    def _extract_companies_from_subset(self, question_text: str) -> list[str]:
        ...
        # Если нет subset_path или файл не существует, используем все файлы в директории с документами
        if self.subset_path is None or not self.subset_path.exists():
            # Получаем все имена файлов в директории документов
            document_files = list(self.documents_dir.glob("*.json"))
            if document_files:
                # Возвращаем имя первого найденного файла без расширения
                return [document_files[0].stem]
            else:
                return ["12345"]  # Фиксированное имя, если нет доступных файлов
        
        # Стандартное извлечение из subset.csv
        if not hasattr(self, 'companies_df'):
            self.companies_df = pd.read_csv(self.subset_path)
        ...
        # Если ничего не найдено, возвращаем первую компанию из списка
        if not found_companies and company_names:
    ...
    def process_comparative_question(self, question: str, companies: List[str], schema: str) -> dict:
        ...
        rephrased_questions = self.bit14_processor.get_rephrased_questions(
        ...
        comparative_answer = self.bit14_processor.get_answer_from_rag_context(
            question=question,
