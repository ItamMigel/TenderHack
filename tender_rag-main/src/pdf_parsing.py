...
class JsonReportProcessor:
    ...
    def assemble_metainfo(self, data):
        ...
        if self.metadata_lookup and sha1_name in self.metadata_lookup:
            ...
            # Если нет метаданных из CSV, используем имя файла как имя компании
            metainfo['company_name'] = sha1_name
...
#     csv_metadata_path=Path("metadata.csv")  # опционально для добавления метаданных
# )

# # Список путей к PDF-файлам
# pdf_paths = [Path("document1.pdf"), Path("document2.pdf")]
