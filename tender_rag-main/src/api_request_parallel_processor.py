...
This script parallelizes requests to the bit14 API while throttling to stay under rate limits.
...
- request_url : str, optional
    ...
    - if omitted, will default to "https://api.bit14.com/v1/embeddings"
- api_key : str, optional
    - API key to use
    - if omitted, the script will attempt to read it from an environment variable {os.getenv("bit14_API_KEY")}
...
class APIRequest:
    ...
    async def call_api(
        ...
        """Calls the bit14 API and saves results."""
...
def api_endpoint_from_url(request_url):
    ...
    if match is None:
        # for Azure bit14 deployment urls
        match = re.search(
            r"^https://[^/]+/bit14/deployments/[^/]+/(.+?)(\?|$)", request_url
        )
    return match[1]
