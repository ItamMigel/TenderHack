...
class Basebit14Processor:
    def __init__(
        self,
        model_path: str = "yandex/YandexGPT-5-Lite-8B-instruct",
        device: str = "cuda:1",
...
class APIProcessor:
    def __init__(self, provider: Literal["bit14", "ibm", "gemini"] ="bit14"):
        self.provider = provider.lower()
        if self.provider == "bit14":
            self.processor = Basebit14Processor()
    ...
    def get_answer_from_rag_context(self, question, rag_context, model, history_chat):
        system_prompt, response_format, user_prompt = self._build_rag_context_prompts("name")
        
        answer_dict = self.processor.send_message(
            model=model,
            system_content=system_prompt,
            human_content=user_prompt.format(context=rag_context, question=question, history_chat=history_chat)
        )
    ...
    def _build_rag_context_prompts(self, schema):
        ...
        if schema == "name":
            system_prompt = prompts.AnswerWithRAGContextNamePrompt.system_prompt
            ...
            system_prompt = prompts.HistoryPrompt.system_prompt
            response_format = prompts.AnswerWithRAGContextNamePrompt.AnswerSchema
            user_prompt = prompts.HistoryPrompt.user_prompt
        elif schema == "boolean":
...
#         request_url="https://api.bit14.com/v1/chat/completions",
